...
# Calling RollingStats function to display the Rolling Mean and Rolling Std for 30 days
...
# Validate the dependency between TRANSACTION_AMOUNT and TRANSACTION_DETAILS attributes [1] [2]
...
#print("Basic VARMAX model start_time is - ", start_time)
...
for index, dfeach in enumerate(ClusterData):
    ...
    df_clusterdata.index = pd.DatetimeIndex(df_clusterdata.index).to_period('M')    # [3]
...
# p_values are [1, 2, 3, 4, 5] and q_values is range(0, 6)
...
for index, dfeach in enumerate(ClusterData):
    ...
    # Calling EvaluateVARMAXTestModel and EvaluateVARMAXFutureModel functions to evaluate model with p and q obtained earlier
...
# [2] https://github.com/nachi-hebbar/Multivariate-Time-Series-Forecasting/blob/main/VAR_Model%20(1).ipynb
