...
for index, dfeach in enumerate(ClusterData):
    ...
    # Calling EvaluateARIMATestModel and EvaluateARIMAFutureModel function to train and predict basic ARIMA model with p,d,q as 3,0,4
    ...
    # Store Root Mean Squared Error for each Cluster
...
# - d will remain as 0 because the dataset is stationary and do not need any differences
...
for index, dfeach in enumerate(ClusterData):
    ...
    # Calling EvaluateARIMATestModel and EvaluateARIMAFutureModel function to train and predict ARIMA model with p,d,q
