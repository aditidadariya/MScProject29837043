...
from keras.layers import Dropout # [1]
...
def CreateRegModel():
    ...
    # Create model object of AdaBoostRegressor [4] 
    models.append(('ABR', AdaBoostRegressor(n_estimators=100, random_state=0)))
    return models

# Define BasicRegModel function to evaluates the models [5]
...
# Define BuildModelRS function to evaluate the models based on the random states [5]
...
def BuildModelCV(models,features,target,randomstate):
    ...
    for name, model in models:
        ...
        # Evaluate each model with cross validation [5] [6]
        cv_results = cross_validate(model, features, target, cv=skfold, scoring=['neg_mean_squared_error','neg_root_mean_squared_error','r2']) 
        # Store the accuracy in results
...
# ADFStationarityTest function is defined to test the time series data for its stationarity using Augmented Dickey-Fuller test [8] [9] [10]
...
# RollingStats function is defined to plot rolling mean and rolling standard deriation of 30 days [11]
def RollingStats(df):
    # Calculate the Rolling Mean and Rolling STD of 30 days
    rolmean = df.rolling(window = 30).mean()    # [12]
    rolstd = df.rolling(window = 30).std()  # [12]
...
# PlotCorrData function is defined to plot the correction plots [11] [13]
def PlotCorrData(df):
...
# EvaluateARIMATestModel function is defined to Evaluate the Basic ARIMA model along observed p,d and q values [14] [15]
...
# EvaluateARIMAFutureModel function is defined to Evaluate the Basic ARIMA model along observed p,d and q values [14] [15]
def EvaluateARIMAFutureModel(df_train, df_test, p, d, q, ind):
    ...
    # Get the future prediction dates  [17]
    ...
    # Converted predicted data into dataframe [16] 
    df_futurepredictions = futurepred.to_frame(name = 'FUTUREPRED_TRANSACTION_AMOUNT')
...
# EvaluateTuneARIMAmodels function is defined to evaluate combinations of p, d and q values for an ARIMA model [18] 
def EvaluateTuneARIMAModels(df_train, df_test, ind, p_values, d_values, q_values):
    ...
    # Retrieving the minimum root mean sqaured error of test and future data [19] 
...
# EvaluateVARMAXTestModel function is defined to evaluate the prediction over test set [20] [36]
...
# EvaluateVARMAXFutureModel function is defined to evaluate the furture prediction [20] [36]
...
# EvaluateTuneVARMAXModels function is defined to evaluate combinations of p, d and q values for an VARMAX model [20] [36]
...
def SplitTrainTestData(df):
    #Spliting the data to 70/30 ratio for train and test data according to the indexes [21]
    df_train = df[0:(int(len(df)*0.7))]
    df_test = df[(int(len(df)*0.7))-1:-1]
    # Convert the train data and test data into array [22]
    trainingset = df_train.to_numpy()
    testset = df_test.to_numpy()
    return trainingset, testset, df_train, df_test

# ProduceGenerator function is defined to get the train and test generators [23]
...
# CompileFitModel function is defined to compile and fit the LSTM Model [24]
def CompileFitModel(model, traingenerator, testgenerator):
    # Defined the EarlyStopping if the loss is not impacted [25]
...
# Define CompileFitTuneModel function to evaluate the model and get the accuracy [24] [26]
...
# TuneRandomSearch function is defined to tune the model using RandomSearch and get best hyperparameters [27] [28] [29]
...
# FinalCompileFitModel function is defined to compile and fit the LSTM Model with best_epoch and best_learning_rate [24]
...
# TestPrediction function is defined to create future predictions using testset [24]
def TestPrediction(trainingset, testset, df_test, model):
    ...
    for i in range(len(testset)):
        ...
        # use the prediction to update the batch and remove the first value [30] 
...
# FuturePrediction function is defined to create future predictions using testset [24] [31]
...
# PlotTestFuturePrediction function is defined to plot the predictions [32]
...
# MultiCompileFitModel function is defined to fit the LSTM Multivariate models [33]
...
# MultiTestPrediction function is defined to make predictions over multivariate test set [32]
...
# MultiFuturePrediction function is defined to make predictions over multivariate future of 4 months [32]
...
# CreateLSTMModel function is defined to create a LSTM model [24] [34]
...
# CreateLSTMModelHP function is defined to create and compile the LSTM model with the hyperparameters [27]
...
# FinalLSTMModel function is defined to create the final lstm model with best_units received [34]
...
# ReshapeCNNData function is defined to reshape the input train and test datasets [35]
...
# CreateCNNModel function is defined to create CNN layers [34] [35]
def CreateCNNModel(training, n_input, n_features):
    ...
    model.add(Conv2D(filters=64, kernel_size=(3,3), padding="same", activation='relu', batch_size = 1, 
                     input_shape=(n_input,1, n_features) ))
...
# CreateCNNModelHP function is defined to create and compile the CNN model with the hyperparameters [35]
...
# FinalCNNModel function is defined to create the final cnn model with best_units received [34] [35]
def FinalCNNModel(trainingset, n_input, n_features, best_units):
    ...
    model.add(Conv2D(filters=best_units, kernel_size=(3,3), padding="same", activation='relu', batch_size = 1, 
                     input_shape=(n_input,1, n_features) ))
    model.add(MaxPooling2D(pool_size=(1,1)))
    model.add(Conv2D(filters=secunits, kernel_size=(3,3), padding="same", activation='relu'))
    model.add(MaxPooling2D(pool_size=(1,1)))
    model.add(Flatten())
    model.add(Dropout(0.5))
    model.add(Dense(50, activation='relu'))
...
# [1] https://www.codegrepper.com/code-examples/whatever/NameError%3A+name+%27Dropout%27+is+not+defined
# [2] https://towardsdatascience.com/how-to-build-your-first-machine-learning-model-in-python-e70fd1907cdd
# [3] https://machinelearningmastery.com/adaboost-ensemble-in-python/
...
# [7] https://www.kaggle.com/code/vitalflux/k-fold-cross-validation-example/notebook
# [8] https://www.hackdeploy.com/augmented-dickey-fuller-test-in-python/#:~:text=The%20adfuller%20function%20returns%20a,a%20dictionary%20of%20Critical%20Values.
# [9] https://machinelearningmastery.com/how-to-develop-an-autoregression-forecast-model-for-household-electricity-consumption/
# [10] https://machinelearningmastery.com/time-series-data-stationary-python/
# [11] https://github.com/llSourcell/Time_Series_Prediction/blob/master/Time%20Series.ipynb
# [12] https://stackoverflow.com/questions/50482884/module-pandas-has-no-attribute-rolling-mean
# [13] https://www.machinelearningplus.com/time-series/arima-model-time-series-forecasting-python/
# [14] https://www.analyticsvidhya.com/blog/2020/10/how-to-create-an-arima-model-for-time-series-forecasting-in-python/
# [15] https://machinelearningmastery.com/arima-for-time-series-forecasting-with-python/
# [16] https://pandas.pydata.org/docs/reference/api/pandas.Series.to_frame.html
# [17] https://stackoverflow.com/questions/57580072/changing-period-to-datetime
# [18] https://machinelearningmastery.com/grid-search-arima-hyperparameters-with-python/
# [19] https://www.geeksforgeeks.org/get-minimum-values-in-rows-or-columns-with-their-index-position-in-pandas-dataframe/#:~:text=a)%20Find%20the%20minimum%20value,the%20values%20in%20the%20series.
# [20] https://github.com/nachi-hebbar/Multivariate-Time-Series-Forecasting/blob/main/VAR_Model%20(1).ipynb
# [21] https://stackoverflow.com/questions/45320659/pandas-how-do-i-get-the-key-index-of-the-first-and-last-row-of-a-dataframe
# [22] https://www.stackvidhya.com/how-to-convert-pandas-dataframe-to-numpy-array/#:~:text=of%20the%20dataframe.-,Convert%20Select%20Columns%20into%20Numpy%20Array,return%20just%20the%20age%20column.
# [23] https://machinelearningmastery.com/how-to-use-the-timeseriesgenerator-for-time-series-forecasting-in-keras/
# [24] https://machinelearningmastery.com/how-to-develop-lstm-models-for-time-series-forecasting/
# [25] https://keras.io/api/callbacks/early_stopping/
# [26] https://machinelearningmastery.com/tune-lstm-hyperparameters-keras-time-series-forecasting/
# [27] https://medium.com/analytics-vidhya/hypertuning-a-lstm-with-keras-tuner-to-forecast-solar-irradiance-7da7577e96eb
# [28] https://towardsdatascience.com/hyperparameter-tuning-the-random-forest-in-python-using-scikit-learn-28d2aa77dd74
# [29] https://machinelearningmastery.com/hyperparameter-optimization-with-random-search-and-grid-search/
# [30] https://www.digitalocean.com/community/tutorials/numpy-append-in-python
# [31] https://machinelearningmastery.com/how-to-develop-lstm-models-for-multi-step-time-series-forecasting-of-household-power-consumption/
# [32] https://stackoverflow.com/questions/56352709/making-matplotlib-pyplot-plot-work-with-periodindex
# [33] https://machinelearningmastery.com/multivariate-time-series-forecasting-lstms-keras/
# [34] https://machinelearningmastery.com/three-ways-to-build-machine-learning-models-in-keras/
# [35] https://machinelearningmastery.com/how-to-develop-convolutional-neural-network-models-for-time-series-forecasting/
# [36] https://machinelearningmastery.com/time-series-forecasting-methods-in-python-cheat-sheet/
