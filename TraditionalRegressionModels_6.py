...
# To implement any model on the data, we need to do feature selection first and therefore the dataset is divided into 2 parts.
# Firstly features (all the attributes except the target attribute) and secondly target
...
# Data is split into training and test set using the feature and target dataframes with holdout method
...
# Spot Check Algorithms with machine learning algorithms
...
# StratifiedKFold Cross Validation is utilized to improve the performance as it splits the data approximately in the same percentage
